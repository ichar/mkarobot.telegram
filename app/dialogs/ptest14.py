...
_RESULTS = {
    'ru' : {
        '01' : [(12, ''), (25, 'Низкий показатель цинизма'), (40, 'Средний показатель цинизма с тенденцией к низкому'), (64, 'Средний показатель цинизма с тенденцией к высокому'), (78, 'Высокий показатель цинизма'),],
        '02' : [(8, ''), (15, 'Низкий показатель агрессивности'), (30, 'Средний показатель агрессивности с тенденцией к низкому'), (45, 'Средний показатель агрессивности с тенденцией к высокому'), (54, 'Высокий показатель агрессивности'),],
        '03' : [(4, ''), (10, 'Низкий показатель враждебности'), (18, 'Средний показатель враждебности с тенденцией к низкому'), (25, 'Средний показатель враждебности с тенденцией к высокому'), (30, 'Высокий показатель враждебности'),],
    },
    'uk' : {
    ...
    if is_run:
        ...
        for p in sorted([x for x in _PARAMS[lang].keys()]):
            if p in _results:
                ...
                storage.set(name, '%s.TP%s' % (_TEST_NAME, p), '%s:%s' % (param, value), with_encode=True)

        if not _is_without_conclusions:
            for conclusion in conclusions.keys():
                value, param = _results[conclusion]
                storage.set(name, '%s.RF' % _TEST_NAME, value)
                storage.set(name, '%s.TF' % _TEST_NAME, '%s:%s' % (param, value), with_encode=True)
